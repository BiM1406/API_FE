...
const userMenuItems = [
  { key: 'my_projects', icon: LayoutDashboard, path: '/dashboard' },
  { key: 'chat_dmp', icon: Terminal, path: '/workspace' },
  { key: 'database', icon: Database, path: '/database' },
  { key: 'test_api', icon: Zap, path: '/test-api' },
  { key: 'collections', icon: Boxes, path: '/collections' },
  { key: 'environments', icon: Network, path: '/environments' },
  { key: 'documentation', icon: BookOpen, path: '/documentation' },
  { key: 'mock_server', icon: Bot, path: '/mock-server' },
  { key: 'history', icon: History, path: '/history' }
];

const adminMenuItems = [
  { key: 'admin_overview', icon: Activity, path: '/admin/overview' },
  { key: 'user_management', icon: Users, path: '/admin/users' },
  { key: 'revenue_management', icon: DollarSign, path: '/admin/revenue' }
];
...
export default function DashboardLayout() {
  ...
  // Khai báo trước handleLogout để tránh temporal dead zone
  const userName = user?.name || t('dashboard.sidebar.default_user');
  const savedPlan = user?.plan?.toLowerCase() || 'free';
  const planDisplay = userRole === 'admin'
  ...
  const handleLogout = () => {
    addActivity({
      ...
      description: t('dashboard.sidebar.logout_desc', { name: userName }),
      status: 'success'
    });
    logout();
    setIsProfileMenuOpen(false);
    ...
    <div className="relative flex h-screen overflow-hidden bg-slate-950 font-sans text-slate-300 selection:bg-indigo-500/30">
      ...
      >
        <div className="h-20 shrink-0 overflow-hidden">
          <div className={`flex h-full items-center ${collapsed ? 'justify-center px-0' : 'justify-between px-5'}`}>
            <button
              ...
              aria-label={collapsed ? t('dashboard.sidebar.expand') : t('dashboard.sidebar.home')}
            >
        ...
        <div className="p-3 border-t border-white/5 overflow-visible relative" ref={profileMenuRef}>
          ...
          {isProfileMenuOpen && (
            <div className={`absolute bottom-full left-3 mb-2 bg-slate-800/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl shadow-black/50 overflow-hidden z-[60] transition-all ${collapsed ? 'w-56' : 'w-[calc(100%-24px)]'}`}>
              ...
              <div className="p-1.5">
                ...
                <button onClick={() => { handleNavigate('/settings'); }} className="w-full text-left px-3 py-2.5 text-xs font-semibold text-slate-300 hover:text-white hover:bg-white/5 rounded-lg transition-all flex items-center gap-3">
          ...
          >
            <div className={`rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 border border-white/10 flex items-center justify-center shrink-0 transition-all font-bold text-white shadow-lg shadow-indigo-500/20 overflow-hidden ${collapsed ? 'w-10 h-10 text-sm' : 'w-9 h-9 text-xs'}`}>
              {user?.avatar ? (
      ...
      >
        <div className="flex h-20 items-center justify-between p-5">
          ...
          <button
            ...
            aria-label={t('dashboard.sidebar.close_mobile')}
          >
            <X size={24} />
          </button>
        </div>
      ...
      <div className="relative z-10 flex min-w-0 flex-1 flex-col overflow-hidden bg-transparent">
        <header className="flex h-16 items-center justify-between border-b border-white/5 px-6 md:hidden">
          ...
          <button
            ...
            aria-label={t('dashboard.sidebar.open_mobile')}
          >
            <Menu size={20} />
          </button>
        </header>
      ...
      <PolicyModal item={supportModalOpen ? 'tech_support' : null} onClose={() => setSupportModalOpen(false)} />
    </div>
  );
}
