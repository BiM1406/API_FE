...
export default function ChatDMP() {
  ...
  const requestedProjectId = projectId || location.state?.projectId || localStorage.getItem('api_fe_active_project_id');
  const [activeId, setActiveId] = useState(requestedProjectId || projects[0]?.id);
  ...
  useEffect(() => {
    if (requestedProjectId && projects.some((project) => project.id === requestedProjectId || project.mpId === requestedProjectId)) {
      const matched = projects.find((project) => project.id === requestedProjectId || project.mpId === requestedProjectId);
    ...
    <div className="flex flex-col flex-1 min-h-0 bg-slate-950 text-slate-300 font-sans overflow-hidden relative w-full">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 relative z-10 w-full">
        ...
        <main className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden relative z-10 bg-black/20">
          ...
          {!['chat', 'env', 'settings'].includes(activeTab) && (
            <div className="flex flex-1 items-center justify-center p-8">
              <div className="w-full max-w-xl rounded-2xl border border-white/5 bg-slate-900/50 p-8 text-center shadow-2xl shadow-black/30">
                <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-indigo-500/20 bg-indigo-500/10 text-indigo-300">
                  <Settings size={24} />
                </div>
                <h3 className="text-xl font-black text-white">{tabs.find((tab) => tab.id === activeTab)?.label}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  Tab này đã được chuẩn bị cho workspace theo project. Có thể nối trực tiếp route/projectId khi tách module ở bước sau.
                </p>
              </div>
            </div>
          )}
