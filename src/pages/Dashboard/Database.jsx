...
export default function Database() {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-slate-950 text-slate-300">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)_460px]">
        ...
        <main className="min-h-0 overflow-y-auto p-4 custom-scrollbar lg:p-6">
          {!activeTable ? (
              ...
              <section className="rounded-2xl border border-white/5 bg-slate-900/45">
                ...
                <div className="overflow-x-auto custom-scrollbar">
                  <div className="min-w-[760px]">
                    <div className="grid grid-cols-[1.2fr_1fr_88px_88px_88px_56px] gap-3 border-b border-white/5 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                      <span>Tên cột</span>
                    ...
                    {activeTable.columns.map((column) => (
                      <div key={column.id} className="grid grid-cols-[1.2fr_1fr_88px_88px_88px_56px] items-center gap-3 border-b border-white/5 px-4 py-3 last:border-b-0">
                        <div className="flex items-center gap-2">
                          {column.primaryKey ? <Key size={15} className="text-amber-300" /> : <span className="h-2 w-2 rounded-full bg-indigo-400" />}
                          <input
                            defaultValue={column.name}
                            onBlur={(event) => handleUpdateColumn(column, { name: event.target.value.trim() })}
                            className="min-w-0 flex-1 rounded-lg border border-white/5 bg-slate-950/60 px-3 py-2 text-sm font-semibold text-white outline-none focus:border-indigo-500/50"
                          />
                        </div>
                        <select
                          value={column.type}
                          onChange={(event) => handleUpdateColumn(column, { type: event.target.value })}
                          className="rounded-lg border border-white/5 bg-slate-950/60 px-3 py-2 text-sm text-slate-200 outline-none focus:border-indigo-500/50"
                        >
                        ...
                        {['primaryKey', 'nullable', 'unique'].map((field) => (
                          <label key={field} className="flex justify-center">
                            ...
                            />
                            <span className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-white/10 bg-slate-950 text-slate-600 transition peer-checked:border-indigo-500/40 peer-checked:bg-indigo-500/20 peer-checked:text-indigo-200">
                              <Check size={15} />
                            </span>
                          </label>
                        ))}
                        <button onClick={() => handleDeleteColumn(column)} className="rounded-lg p-2 text-slate-500 transition hover:bg-red-500/10 hover:text-red-300">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    ))}
        ...
        <aside className="min-h-0 border-t border-white/5 bg-black/20 xl:border-l xl:border-t-0">
