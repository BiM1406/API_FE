...
export default function Register({ onSwitch, onRegisterSuccess }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const newErrors = {};
    if (!name.trim()) newErrors.name = 'Vui lòng nhập tên tài khoản';
    if (!email.trim()) newErrors.email = 'Vui lòng nhập email';
    else if (!isEmail(email)) newErrors.email = 'Email không hợp lệ';
    ...
    try {
      ...
      toast.success('Hãy kiểm tra email để lấy mã xác thực! Mã mock: 123456');
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        >
          {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Đăng Ký <ArrowRight className="w-4 h-4" /></>}
        </motion.button>
      </form>
